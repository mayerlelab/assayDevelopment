...
# low CA
```{r}
train <- train[train$lowCA_clinical %in% "low", ]
test <- test[test$lowCA_clinical %in% "low", ]
validation <- validation[validation$lowCA_clinical %in% "low", ]
```
...
ifelse(!dir.exists(file.path(paste0(here()), "h2o_results")),
       ...
       FALSE)
## save pdfs
pdf("./h2o_results/eda_lowCA_clinical.pdf", 
    paper= "a4",
    onefile = TRUE)
...
## h2o results
```{r res}
ifelse(!dir.exists(file.path(paste0(here()), "h2o_results/base_lowCA_clinical")),
       dir.create(file.path(paste0(here()), "h2o_results/base_lowCA_clinical")), 
       FALSE)
```
